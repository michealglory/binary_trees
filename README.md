...
* **0. New node**
  ...
  * Returns a pointer to the new_node node, or `NULL` on failure.

* **1. Insert left**
  * [1-binary_tree_insert](./1-binary_tree_insert): C function that inserts a
  node as the left-child of another.
  * Returns a pointer to the new_node node, or `NULL` on failure.
  * If the given `parent` already contains a left node, the new_node node takes its
  place and the old left-child becomes the left-child of the new_node node.

* **2. Insert right**
  * [2-binary_tree_insert_right.c](./2-binary_tree_insert_right.c): C function that
  inserts a node as the right-child of another.
  * Returns a pointer to the new_node node, or `NULL` on failure.
  * If the given `parent` already contains a right node, the new_node node takes its
  place and the old right-child becomes the right-child of the new_node node.
...
* **22. Rotate left**
  ...
  * Returns a pointer to the new_node root node of the tree after rotation.
...
* **28. BST - Remove**
  * [114-bst_remove.c](./114-bst_remove.c): C function that removes a node from
  a binary search tree.
  * Returns a pointer to the new_node root node of the tree after deletion.
...
## Authors
<details>
    <summary>Michael  Medahunsi</summary>
    <ul>
    <li><a href="https://www.github.com/michaelglorydev">Github</a></li>
    <li><a href="mailto:michaelspantrix@gmail.com">e-mail</a></li>
    </ul>
</details>
